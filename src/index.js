...
function validateLine(entry, data, index = null) {
  ...
  const strategy = {
    ...
    [exceptionTypes.JSON]: (data, entry) => {
      const { enumOps } = entry;
       ...
       */
       toDomain: (data) => {
        const convert = (entry) => {
          ...
          for (const key of keys) {
            const founded = dto.find((dt) => dt.name === key);
            
            if (founded) {
              let value = entry[key]
              if (founded.type === 'Date') {
                if (entry[key] && entry[key].seconds){
                  value = new Date(entry[key].seconds * 1000)
                } else if (entry[key] && entry[key]._seconds){
                  value = new Date(entry[key]._seconds * 1000)
                } else {
                  value = new Date(entry[key])
                }
              } else if (founded.type === 'JSON') {
              ...
              serialize[founded.serialize] = value;
            }
          }
